<script lang="ts">
	...
	import Navbar from '$features/chat/Navbar.svelte';
	import ChatControls from './ChatControls.svelte';
	import EventConfirmDialog from '$shared/components/ConfirmDialog.svelte';
	import Placeholder from './Placeholder.svelte';
	import NotificationToast from '../app/NotificationToast.svelte';
	...
	const uploadGoogleDriveFile = async (fileData) => {
		...
		try {
			...
			fileItem.url = `${JYOTIGPT_API_BASE_URL}/files/${uploadedFile.id}`;
...
<svelte:head>
	<title>
		{$chatTitle
			? `${$chatTitle.length > 30 ? `${$chatTitle.slice(0, 30)}...` : $chatTitle} | ${$JYOTIGPT_NAME}`
			: `${$JYOTIGPT_NAME}`}
	</title>
</svelte:head>
